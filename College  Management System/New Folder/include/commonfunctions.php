...
// long binary data?
	...
	else if($format==FORMAT_FILE)
	{
		...
		if($strTableName=="assignments")
		{
			$iquery.="&key1=".rawurlencode($data["AID"]);
		}
		if($strTableName=="attendance")
		{
			$iquery.="&key1=".rawurlencode($data["ATID"]);
		}
		if($strTableName=="classes")
		{
			$iquery.="&key1=".rawurlencode($data["CID"]);
		}
		if($strTableName=="lectures")
		{
			$iquery.="&key1=".rawurlencode($data["LID"]);
		}
		if($strTableName=="marks")
		{
			$iquery.="&key1=".rawurlencode($data["MID"]);
		}
		if($strTableName=="students")
		{
			$iquery.="&key1=".rawurlencode($data["SID"]);
		}
		if($strTableName=="teachers")
		{
			$iquery.="&key1=".rawurlencode($data["TID"]);
		}
...
//	assignments
	...
	{
			$value=make_db_value("AID",$keys["AID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("AID")." is null";
		else
			$strWhere.=GetFullFieldName("AID")."=".make_db_value("AID",$keys["AID"]);
	}

//	attendance
	if($table=="attendance")
	{
			$value=make_db_value("ATID",$keys["ATID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("ATID")." is null";
		else
			$strWhere.=GetFullFieldName("ATID")."=".make_db_value("ATID",$keys["ATID"]);
	}

//	classes
	if($table=="classes")
	{
			$value=make_db_value("CID",$keys["CID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("CID")." is null";
		else
			$strWhere.=GetFullFieldName("CID")."=".make_db_value("CID",$keys["CID"]);
	}

//	lectures
	if($table=="lectures")
	{
			$value=make_db_value("LID",$keys["LID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("LID")." is null";
		else
			$strWhere.=GetFullFieldName("LID")."=".make_db_value("LID",$keys["LID"]);
	}

//	marks
	if($table=="marks")
	{
			$value=make_db_value("MID",$keys["MID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("MID")." is null";
		else
			$strWhere.=GetFullFieldName("MID")."=".make_db_value("MID",$keys["MID"]);
	}

//	students
	if($table=="students")
	{
			$value=make_db_value("SID",$keys["SID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("SID")." is null";
		else
			$strWhere.=GetFullFieldName("SID")."=".make_db_value("SID",$keys["SID"]);
	}

//	teachers
	if($table=="teachers")
	{
			$value=make_db_value("TID",$keys["TID"]);
				if($value==="null")
			$strWhere.=GetFullFieldName("TID")." is null";
		else
			$strWhere.=GetFullFieldName("TID")."=".make_db_value("TID",$keys["TID"]);
	}
...
{
	global $strTableName,$filename,$files_delete,$files_move,$files_save;
	$filename="";
	$type=GetFieldType($field,$table);
...
//	build SQL strings for lookup wizards
		if($strTableName=="assignments" && $field=="LID") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
			$LookupTable="lectures";
			$strSize=1;
		...
		if($strTableName=="attendance" && $field=="SID") 
		{
			$strPerm = GetUserPermissions("students");
					$LinkField="SID";
			...
			$LookupSQL = "select ";
						$LookupSQL .= "`SID`";
						$LookupSQL .= ",`Full_Name`";
						$LookupSQL .= " from `students` ";
			
						
			
		}
		if($strTableName=="attendance" && $field=="LRID") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `lectures`.`Name`";
			
		}
		if($strTableName=="classes" && $field=="Semester") 
		{
		...
		if($strTableName=="classes" && $field=="Subjects") 
		{
			$strPerm = GetUserPermissions("");
					$LinkField="";
			$DisplayField="";
		...
		if($strTableName=="lectures" && $field=="CID") 
		{
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `classes`.`CID`";
			
		}
		if($strTableName=="lectures" && $field=="TID") 
		{
			$strPerm = GetUserPermissions("teachers");
					$LinkField="TID";
			$DisplayField="Full_Name";
			...
			$LookupSQL = "select ";
						...
						$LookupSQL.=" ORDER BY `teachers`.`Full_Name`";
			
		}
		if($strTableName=="lectures" && $field=="Type") 
		{
			$strPerm = GetUserPermissions("");
					$LinkField="";
			$DisplayField="";
		...
		if($strTableName=="marks" && $field=="SID") 
		{
			$strPerm = GetUserPermissions("students");
					$LinkField="SID";
			$DisplayField="Full_Name";
		...
		if($strTableName=="marks" && $field=="LID") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="CID") 
		{
			$strPerm = GetUserPermissions("classes");
					$LinkField="CID";
			$DisplayField="CID";
		...
		if($strTableName=="students" && $field=="L1") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L2") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L3") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L4") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L5") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L6") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
		...
		if($strTableName=="students" && $field=="L7") 
		{
			$strPerm = GetUserPermissions("lectures");
					$LinkField="LID";
			$DisplayField="Name";
			...
			//	add new item
				if($mode!=MODE_SEARCH && $addnewitem || $lookuptype==LCT_LIST)
				{
					...
					if($addnewitem)
					{
						if(!$advancedadd)
						{
							echo "<a href=# onclick=\"window.open('assignments_addnewitem.php?".
...
//	get the initial value
			...
			if($strTableName=="assignments" && $field=="LID") 
			{
								...
								$lookup_SQL .= "`LID`";
								$lookup_SQL .= ",`Name`";
				$lookup_SQL .= " FROM `lectures` ";
								$lookup_SQL .= " WHERE `LID`=".make_db_value($field,$value)."";
							}
			if($strTableName=="attendance" && $field=="SID") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`SID`";
								$lookup_SQL .= ",`Full_Name`";
				$lookup_SQL .= " FROM `students` ";
								$lookup_SQL .= " WHERE `SID`=".make_db_value($field,$value)."";
							}
			if($strTableName=="attendance" && $field=="LRID") 
			{
								$lookup_SQL = "SELECT ";
			...
			if($strTableName=="lectures" && $field=="CID") 
			{
								...
								$lookup_SQL .= "`CID`";
								$lookup_SQL .= ",`CID`";
				$lookup_SQL .= " FROM `classes` ";
								$lookup_SQL .= " WHERE `CID`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `CID`";
								}
			if($strTableName=="lectures" && $field=="TID") 
			{
								$lookup_SQL = "SELECT ";
								$lookup_SQL .= "`TID`";
								$lookup_SQL .= ",`Full_Name`";
				$lookup_SQL .= " FROM `teachers` ";
								$lookup_SQL .= " WHERE `TID`=".make_db_value($field,$value)."";
									$lookup_SQL.= " ORDER BY `Full_Name`";
								}
			if($strTableName=="lectures" && $field=="Type") 
			{
							}
...
function BuildEditControl($field , $value, $format, $edit, $secondfield=false, $id="")
{
	...
	if($strTableName=="assignments")
	{
		$keylink.="&key1=".rawurlencode(@$keys["AID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="attendance")
	{
		$keylink.="&key1=".rawurlencode(@$keys["ATID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="classes")
	{
		$keylink.="&key1=".rawurlencode(@$keys["CID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="lectures")
	{
		$keylink.="&key1=".rawurlencode(@$keys["LID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="marks")
	{
		$keylink.="&key1=".rawurlencode(@$keys["MID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="students")
	{
		$keylink.="&key1=".rawurlencode(@$keys["SID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="teachers")
	{
		$keylink.="&key1=".rawurlencode(@$keys["TID"]);
		$iquery.=$keylink;
	}
	if($strTableName=="users")
	{
