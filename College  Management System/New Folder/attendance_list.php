...
//	this code must be executed after the inlineedit.js is loaded
	...
	$code_end.='
		window.inlineEditing'.$id.' = new InlineEditing(\'attendance\',\'php\','.$id.');
		'.$afteredited_handler;
...
{
	$includes_js[]="include/inlineedit.js";
	$code_end .= 'window.inlineEditing'.$id.' = new InlineEditing(\'attendance\',\'php\');';
}
...
//	validation stuff
	...
	$addValidateFields = array();
			...
			$editValidateFields[] = "SID";
					...
					$validatetype.="IsRequired";
			$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "LRID";
						$validatetype="";
					$validatetype.="IsRequired";
			$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "Daate";
...
"\nvar bLoading=false;\r\n";
	$code_begin.="var SUGGEST_TABLE='attendance_searchsuggest.php';\r\n";
	$code_begin.="var MASTER_PREVIEW_TABLE='attendance_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"attendance_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"attendance_list.php\">";
...
$href="attendance_list.php?orderby=aSID";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="attendance_list.php?orderby=aLRID";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("LRID_orderlinkattrs",$orderlinkattrs);
$xt->assign("LRID_fieldheader",true);
$href="attendance_list.php?orderby=aDaate";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("Daate_orderlinkattrs",$orderlinkattrs);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$href="attendance_list.php?orderby=".$dir.GoodFieldName($order_field);
	$orderlinkattrs="";
	if($mode==LIST_LOOKUP)
	{
		$href.="&".$lookupparams;
...
{  
	...
	else
	{
		...
		if($where=StrWhere("SID", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("LRID", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Daate", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$code_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='attendance_list.php?goto='+nPageNumber;
				};";
		else
			$code_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='attendance_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	detail tables
			...
			$detailid[]=make_db_value("LRID",$data["LRID"],"","","lectures");
	//	add count of child records to SQL
		if(!$bSubqueriesSupported)
	{
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["ATID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["ATID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["ATID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["ATID"]));

	$record["editlink_attrs"]="href=\"attendance_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"attendance_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"attendance_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"attendance_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	Daate - Short Date
			$value="";
				$value = ProcessLargeText(GetData($data,"Daate", "Short Date"),"field=Daate".$keylink,"",MODE_LIST);
			...
			$span="<span ";
			$span.="id=\"edit".$recid."_LRID\" ";
					$span.="val=\"".htmlspecialchars($data["LRID"])."\" ";
			$span.=">";
			$record["LRID_value"] = $span.$record["LRID_value"]."</span>";
			$span="<span ";
			...
			$span="<span ";
			$span.="id=\"edit".$recid."_Daate\" ";
					$span.="val=\"".htmlspecialchars($data["Daate"])."\" ";
			$span.=">";
			$record["Daate_value"] = $span.$record["Daate_value"]."</span>";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"attendance_print.php\" onclick=\"window.open('attendance_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"attendance_print.php?all=1\" onclick=\"window.open('attendance_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"attendance_export.php\" onclick=\"window.open('attendance_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='attendance_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='attendance_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"attendance_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='attendance_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='attendance_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"attendance_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"attendance_add.php\" onClick=\"window.location.href='attendance_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"attendance_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'attendance_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"attendance_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"attendance_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'attendance_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("asearchlink_attrs","href=\"attendance_search.php\" onclick=\"window.location.href='attendance_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"attendance_import.php\" onclick=\"window.location.href='attendance_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"attendance_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"attendance_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='attendance_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='attendance_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "attendance_list.htm";
