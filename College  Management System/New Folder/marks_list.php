...
include("include/marks_variables.php");
	include("include/lectures_settings.php");
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$code_end.='
		window.inlineEditing'.$id.' = new InlineEditing(\'marks\',\'php\','.$id.');
		'.$afteredited_handler;
...
{
	$includes_js[]="include/inlineedit.js";
	$code_end .= 'window.inlineEditing'.$id.' = new InlineEditing(\'marks\',\'php\');';
}
...
//	validation stuff
	...
	$addValidateFields = array();
			...
			$editValidateFields[] = "SID";
						$validatetype="";
					$validatetype.="IsRequired";
			$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "LID";
										$validatetype="IsNumeric";
					$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "SESSINAL";
										$validatetype="IsNumeric";
					$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "MIDTERM";
										$validatetype="IsNumeric";
					$editValidateTypes[] = $validatetype;
			$editValidateFields[] = "FINAL";
	
						$validatetype="";
					$validatetype.="IsRequired";
			$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "SID";
						$validatetype="";
					$validatetype.="IsRequired";
			$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "LID";
										$validatetype="IsNumeric";
					$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "SESSINAL";
										$validatetype="IsNumeric";
					$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "MIDTERM";
										$validatetype="IsNumeric";
					$addValidateTypes[] = $validatetype;
			$addValidateFields[] = "FINAL";
...
"\nvar bLoading=false;\r\n";
	$code_begin.="var SUGGEST_TABLE='marks_searchsuggest.php';\r\n";
	$code_begin.="var MASTER_PREVIEW_TABLE='marks_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"marks_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"marks_list.php\">";
...
$href="marks_list.php?orderby=aSID";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$xt->assign("SID_orderlinkattrs",$orderlinkattrs);
$xt->assign("SID_fieldheader",true);
$href="marks_list.php?orderby=aLID";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("LID_orderlinkattrs",$orderlinkattrs);
$xt->assign("LID_fieldheader",true);
$href="marks_list.php?orderby=aSESSINAL";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("SESSINAL_orderlinkattrs",$orderlinkattrs);
$xt->assign("SESSINAL_fieldheader",true);
$href="marks_list.php?orderby=aMIDTERM";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("MIDTERM_orderlinkattrs",$orderlinkattrs);
$xt->assign("MIDTERM_fieldheader",true);
$href="marks_list.php?orderby=aFINAL";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("FINAL_orderlinkattrs",$orderlinkattrs);
$xt->assign("FINAL_fieldheader",true);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$href="marks_list.php?orderby=".$dir.GoodFieldName($order_field);
	$orderlinkattrs="";
	if($mode==LIST_LOOKUP)
	{
		$href.="&".$lookupparams;
...
{  
	...
	{
		$strWhere = "1=0";
		if($where=StrWhere("SID", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("LID", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("SESSINAL", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("MIDTERM", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("FINAL", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
...
if($allow_search)
{
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
			$strOriginalDetailsTable="lectures";
	...
	$countsql = "select count(*) from ".AddTableWrappers($strOriginalDetailsTable)." where ".$sqlWhere;
		$gsqlHead.=",(".$countsql.") as lectures_cnt ";
	$sqlWhere="";
	$masterkeys=array();
	$masterkeys[]="`SID`";
	$detailkeys=array();
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$code_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='marks_list.php?goto='+nPageNumber;
				};";
		else
			$code_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='marks_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//detail tables
		$record["lectures_dtable_link"]=$allow_lectures;
		$record["lectures_dtablelink_attrs"]=" href=\"lectures_list.php?\" id=\"master_lectures_add".$id."\"";
			$record["lectures_dtablelink_attrs"].=" onmouseover=\"RollDetailsLink.showPopup(this,'lectures_detailspreview.php'+this.href.substr(this.href.indexOf('?')));\" onmouseout=\"RollDetailsLink.hidePopup();\"";
	//detail tables
		$record["students_dtable_link"]=$allow_students;
		$record["students_dtablelink_attrs"]=" href=\"students_list.php?\" id=\"master_students_add".$id."\"";
			...
			$record["SID_value"] = "<span id=\"add".$id."_SID\">&nbsp;</span>";
				if(!$allow_edit || $mode==LIST_LOOKUP)
			$record["SID_value"] = "<span id=\"ieditlink_add".$id."\"></span>".$record["SID_value"];
...
//	add spans for link and display fieldsif they don't appear on the List page		
		...
		$record["LID_value"] = "<span id=\"add".$id."_LID\">&nbsp;</span>";
//	add spans for link and display fieldsif they don't appear on the List page		
		if($mode==LIST_LOOKUP && $lookupSelectField)
		{
			$span="";
		...
		$record["SESSINAL_value"] = "<span id=\"add".$id."_SESSINAL\">&nbsp;</span>";
//	add spans for link and display fieldsif they don't appear on the List page		
		if($mode==LIST_LOOKUP && $lookupSelectField)
		{
			$span="";
		...
		$record["MIDTERM_value"] = "<span id=\"add".$id."_MIDTERM\">&nbsp;</span>";
//	add spans for link and display fieldsif they don't appear on the List page		
		if($mode==LIST_LOOKUP && $lookupSelectField)
		{
			$span="";
		...
		$record["FINAL_value"] = "<span id=\"add".$id."_FINAL\">&nbsp;</span>";
//	add spans for link and display fieldsif they don't appear on the List page		
		if($mode==LIST_LOOKUP && $lookupSelectField)
		{
			$span="";
...
//	detail tables
			...
			$detailid[]=make_db_value("LID",$data["LID"],"","","lectures");
	//	add count of child records to SQL
		if(!$bSubqueriesSupported)
	{
	$sqlHead="SELECT LID,  CID,  TID,  Name,  `Continue`,  `Type`,  `Start`,  `End`,  `Time`,  Room,  Duration,  About,  Announce ";
	$sqlFrom="FROM lectures ";
	$sqlWhere="";
	$sqlTail="";
	$masterkeys=array();
	$masterkeys[]="`LID`";
	...
	foreach($masterkeys as $idx=>$val)
	{
		if($masterwhere)
			$masterwhere.=" and ";
	$masterwhere.=GetFullFieldName($detailkeys[$idx],"lectures")."=".$detailid[$idx];
	}
	$data["lectures_cnt"]=gSQLRowCount_int($sqlHead,$sqlFrom,$sqlWhere,$sqlTail,$masterwhere,0);
	}

//detail tables
	$record["lectures_dtable_link"]=$allow_lectures;
	$record["lectures_dtablelink_attrs"]="href=\"lectures_list.php?".$masterquery."\" id=\"master_lectures".$recid."\"";
	$dpreview=true;
			if(!($data["lectures_cnt"]+0))
	{
		$record["lectures_dtable_link"]=false;
		$dpreview=false;
	}
	if($dpreview)
	{
			...
			$detailid[]=make_db_value("SID",$data["SID"],"","","students");
	//	add count of child records to SQL
		if(!$bSubqueriesSupported)
	{
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["MID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["MID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["MID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["MID"]));

	$record["editlink_attrs"]="href=\"marks_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"marks_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"marks_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"marks_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	LID - 
			...
			$record["LID_value"]=$value;

//	SESSINAL - 
			$value="";
				$value = ProcessLargeText(GetData($data,"SESSINAL", ""),"field=SESSINAL".$keylink,"",MODE_LIST);
			$record["SESSINAL_value"]=$value;

//	MIDTERM - 
			$value="";
				$value = ProcessLargeText(GetData($data,"MIDTERM", ""),"field=MIDTERM".$keylink,"",MODE_LIST);
			$record["MIDTERM_value"]=$value;

//	FINAL - 
			$value="";
				$value = ProcessLargeText(GetData($data,"FINAL", ""),"field=FINAL".$keylink,"",MODE_LIST);
			...
			$span="<span ";
			$span.="id=\"edit".$recid."_SID\" ";
					$span.="val=\"".htmlspecialchars($data["SID"])."\" ";
			$span.=">";
			$record["SID_value"] = $span.$record["SID_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_LID\" ";
					$span.="val=\"".htmlspecialchars($data["LID"])."\" ";
			$span.=">";
			$record["LID_value"] = $span.$record["LID_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_SESSINAL\" ";
					$span.="val=\"".htmlspecialchars($data["SESSINAL"])."\" ";
			$span.=">";
			$record["SESSINAL_value"] = $span.$record["SESSINAL_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_MIDTERM\" ";
					$span.="val=\"".htmlspecialchars($data["MIDTERM"])."\" ";
			$span.=">";
			$record["MIDTERM_value"] = $span.$record["MIDTERM_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_FINAL\" ";
					$span.="val=\"".htmlspecialchars($data["FINAL"])."\" ";
			$span.=">";
			$record["FINAL_value"] = $span.$record["FINAL_value"]."</span>";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"marks_print.php\" onclick=\"window.open('marks_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"marks_print.php?all=1\" onclick=\"window.open('marks_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"marks_export.php\" onclick=\"window.open('marks_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='marks_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='marks_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"marks_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='marks_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='marks_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"marks_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"marks_add.php\" onClick=\"window.location.href='marks_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"marks_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'marks_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"marks_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
	...
	if($mode!=LIST_LOOKUP)
	{
 		$xt->assign("lectures_dtable_column",$allow_lectures);
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"marks_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'marks_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("SESSINAL_fieldheadercolumn",true);
$xt->assign("SESSINAL_fieldcolumn",true);
$xt->assign("SESSINAL_fieldfootercolumn",true);
$xt->assign("MIDTERM_fieldheadercolumn",true);
$xt->assign("MIDTERM_fieldcolumn",true);
$xt->assign("MIDTERM_fieldfootercolumn",true);
$xt->assign("FINAL_fieldheadercolumn",true);
$xt->assign("FINAL_fieldcolumn",true);
$xt->assign("FINAL_fieldfootercolumn",true);
	
$display_grid = $allow_add || $allow_search && $rowsfound;

$xt->assign("asearch_link",$allow_search);
$xt->assign("asearchlink_attrs","href=\"marks_search.php\" onclick=\"window.location.href='marks_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"marks_import.php\" onclick=\"window.location.href='marks_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"marks_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"marks_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='marks_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='marks_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
