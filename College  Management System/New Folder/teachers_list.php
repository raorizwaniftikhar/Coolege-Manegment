...
{	
	...
	if($lookuptable=="lectures" && $lookupfield=="TID") 
	{
		$linkfield = "TID";
		$dispfield = "Full_Name";
			}
...
//	this code must be executed after the inlineedit.js is loaded
	...
	$code_end.='
		window.inlineEditing'.$id.' = new InlineEditing(\'teachers\',\'php\','.$id.');
		'.$afteredited_handler;
...
{
	$includes_js[]="include/inlineedit.js";
	$code_end .= 'window.inlineEditing'.$id.' = new InlineEditing(\'teachers\',\'php\');';
}
...
//	validation stuff
	...
	$addValidateFields = array();
		...
		$editValidateFields[] = "Full_Name";
			$editValidateTypes[] = "";
		$editValidateFields[] = "Designation";
			$editValidateTypes[] = "";
		$editValidateFields[] = "Phone";
			$editValidateTypes[] = "";
		$editValidateFields[] = "Address";
	
			$addValidateTypes[] = "";
		$addValidateFields[] = "Full_Name";
			$addValidateTypes[] = "";
		$addValidateFields[] = "Designation";
			$addValidateTypes[] = "";
		$addValidateFields[] = "Phone";
			$addValidateTypes[] = "";
		$addValidateFields[] = "Address";
...
"\nvar bLoading=false;\r\n";
	$code_begin.="var SUGGEST_TABLE='teachers_searchsuggest.php';\r\n";
	$code_begin.="var MASTER_PREVIEW_TABLE='teachers_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"teachers_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"teachers_list.php\">";
...
$href="teachers_list.php?orderby=aFull_Name";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="teachers_list.php?orderby=aDesignation";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("Designation_orderlinkattrs",$orderlinkattrs);
$xt->assign("Designation_fieldheader",true);
$href="teachers_list.php?orderby=aPhone";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("Phone_orderlinkattrs",$orderlinkattrs);
$xt->assign("Phone_fieldheader",true);
$href="teachers_list.php?orderby=aAddress";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("Address_orderlinkattrs",$orderlinkattrs);
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$href="teachers_list.php?orderby=".$dir.GoodFieldName($order_field);
	$orderlinkattrs="";
	if($mode==LIST_LOOKUP)
	{
		$href.="&".$lookupparams;
...
{  
	...
	else
	{
		...
		if($where=StrWhere("UID", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Designation", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Phone", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Address", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$code_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='teachers_list.php?goto='+nPageNumber;
				};";
		else
			$code_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='teachers_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["TID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["TID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["TID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["TID"]));

	$record["editlink_attrs"]="href=\"teachers_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"teachers_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"teachers_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"teachers_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	Designation - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Designation", ""),"field=Designation".$keylink,"",MODE_LIST);
			$record["Designation_value"]=$value;

//	Phone - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Phone", ""),"field=Phone".$keylink,"",MODE_LIST);
			$record["Phone_value"]=$value;
...
//	Address - 
			...
			$span="<span ";
			$span.="id=\"edit".$recid."_Designation\" ";
					$span.="val=\"".htmlspecialchars($data["Designation"])."\" ";
			$span.=">";
			$record["Designation_value"] = $span.$record["Designation_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_Phone\" ";
					$span.="val=\"".htmlspecialchars($data["Phone"])."\" ";
			$span.=">";
			$record["Phone_value"] = $span.$record["Phone_value"]."</span>";
			$span="<span ";
			$span.="id=\"edit".$recid."_Address\" ";
					$span.="val=\"".htmlspecialchars($data["Address"])."\" ";
			$span.=">";
			$record["Address_value"] = $span.$record["Address_value"]."</span>";
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"teachers_print.php\" onclick=\"window.open('teachers_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"teachers_print.php?all=1\" onclick=\"window.open('teachers_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"teachers_export.php\" onclick=\"window.open('teachers_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='teachers_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='teachers_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"teachers_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='teachers_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='teachers_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"teachers_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"teachers_add.php\" onClick=\"window.location.href='teachers_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"teachers_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'teachers_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"teachers_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"teachers_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'teachers_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("Designation_fieldheadercolumn",true);
$xt->assign("Designation_fieldcolumn",true);
$xt->assign("Designation_fieldfootercolumn",true);
...
$xt->assign("asearchlink_attrs","href=\"teachers_search.php\" onclick=\"window.location.href='teachers_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"teachers_import.php\" onclick=\"window.location.href='teachers_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"teachers_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"teachers_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='teachers_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='teachers_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "teachers_list.htm";
