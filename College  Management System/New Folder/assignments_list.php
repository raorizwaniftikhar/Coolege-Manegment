...
//	this code must be executed after the inlineedit.js is loaded
	...
	$code_end.='
		window.inlineEditing'.$id.' = new InlineEditing(\'assignments\',\'php\','.$id.');
		'.$afteredited_handler;
...
"\nvar bLoading=false;\r\n";
	$code_begin.="var SUGGEST_TABLE='assignments_searchsuggest.php';\r\n";
	$code_begin.="var MASTER_PREVIEW_TABLE='assignments_masterpreview.php';\r\n";
...
$body = array();
if($mode==LIST_SIMPLE)
	$html_begin.="<form name=\"frmSearch\" method=\"GET\" action=\"assignments_list.php\">";
else
{
	$html_begin.="<form name=\"frmSearch".$id."\" target=\"flyframe".$id."\" method=\"GET\" action=\"assignments_list.php\">";
...
$href="assignments_list.php?orderby=aTitle";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$xt->assign("Title_orderlinkattrs",$orderlinkattrs);
$xt->assign("Title_fieldheader",true);
$href="assignments_list.php?orderby=aMarks";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("Marks_orderlinkattrs",$orderlinkattrs);
$xt->assign("Marks_fieldheader",true);
$href="assignments_list.php?orderby=aDescription";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$href="assignments_list.php?orderby=aFile";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
$orderlinkattrs.=" href=\"".$href."\"";
$xt->assign("File_orderlinkattrs",$orderlinkattrs);
$xt->assign("File_fieldheader",true);
$href="assignments_list.php?orderby=aLID";
$orderlinkattrs="";
if($mode==LIST_LOOKUP)
{
	$href.="&".$lookupparams;
...
if(@$_SESSION[$strTableName."_orderby"])
{
	...
	$href="assignments_list.php?orderby=".$dir.GoodFieldName($order_field);
	$orderlinkattrs="";
	if($mode==LIST_LOOKUP)
	{
		$href.="&".$lookupparams;
...
{  
	...
	else
	{
		...
		if($where=StrWhere("Title", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Marks", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("Description", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
		if($where=StrWhere("File", $strSearchFor, $strSearchOption, ""))
			$strWhere .= " or ".$where;
	...
	if($maxpages>1)
	{
		...
		if($mode==LIST_SIMPLE)
			$code_end.="window.GotoPage = function(nPageNumber)
				{
					window.location='assignments_list.php?goto='+nPageNumber;
				};";
		else
			$code_end.="window.GotoPage".$id." = function (nPageNumber)
				{
					window.frames['flyframe".$id."'].location='assignments_list.php?".$lookupparams."&goto='+nPageNumber;
				};";
...
//	detail tables
			$masterquery="mastertable=assignments";
...
//	key fields
	...
	$copylink="";
	$keylink="";
	$keyblock.= rawurlencode($data["AID"]);
	$editlink.="editid1=".htmlspecialchars(rawurlencode($data["AID"]));
	$copylink.="copyid1=".htmlspecialchars(rawurlencode($data["AID"]));
	$keylink.="&key1=".htmlspecialchars(rawurlencode(@$data["AID"]));

	$record["editlink_attrs"]="href=\"assignments_edit.php?".$editlink."\" id=\"editlink".$recid."\"";
	$record["inlineeditlink_attrs"]= "href=\"assignments_edit.php?".$editlink."\" onclick=\"return inlineEditing".$id.".inlineEdit('".$recid."','".$editlink."');\" id=\"ieditlink".$recid."\"";
	$record["copylink_attrs"]="href=\"assignments_add.php?".$copylink."\" id=\"copylink".$recid."\"";
	$record["viewlink_attrs"]="href=\"assignments_view.php?".$editlink."\" id=\"viewlink".$recid."\"";
	if($mode!=LIST_LOOKUP)
	{
...
//	Marks - 
			$value="";
				$value = ProcessLargeText(GetData($data,"Marks", ""),"field=Marks".$keylink,"",MODE_LIST);
			$record["Marks_value"]=$value;
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("printlink_attrs","href=\"assignments_print.php\" onclick=\"window.open('assignments_print.php','wPrint');return false;\"");
	$xt->assign("printalllink_attrs","href=\"assignments_print.php?all=1\" onclick=\"window.open('assignments_print.php?all=1','wPrint');return false;\"");
	$xt->assign("export_link",$allow_export);
	$xt->assign("exportlink_attrs","href=\"assignments_export.php\" onclick=\"window.open('assignments_export.php','wExport');return false;\"");
	...
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='assignments_print.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='assignments_list.php'; 
	document.forms.frmAdmin.target='_self';return false\"
	href=\"assignments_print.php\"");
	$xt->assign("exportselected_link",$allow_export);
	$xt->assign("exportselectedlink_attrs","disptype=\"control1\" onclick=\"
	if(!\$('input[@type=checkbox][@checked][@name^=selection]').length)
		return true;
	document.forms.frmAdmin.action='assignments_export.php';
	document.forms.frmAdmin.target='_blank';
	document.forms.frmAdmin.submit(); 
	document.forms.frmAdmin.action='assignments_list.php'; 
	document.forms.frmAdmin.target='_self';return false;\"
	href=\"assignments_export.php\"");
	
	$xt->assign("add_link",$allow_add);
	$xt->assign("copy_column",$allow_add);
	$xt->assign("addlink_attrs","href=\"assignments_add.php\" onClick=\"window.location.href='assignments_add.php'\"");
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"assignments_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,null,'assignments_add.php');\"");
...
for (i=0;i<frmAdmin.elements['selection[]'].length;++i) 
	...
	$xt->assign("editselectedlink_attrs","href=\"assignments_edit.php\" disptype=\"control1\" name=\"edit_selected".$id."\" onclick=\"\$('input[@type=checkbox][@checked][@id^=check]').each(function(i){
...
//	$xt->assign("checkbox_column",true);
	$xt->assign("inlineadd_link",$allow_add);
	$xt->assign("inlineaddlink_attrs","href=\"assignments_add.php\" onclick=\"return inlineEditing".$id.".inlineAdd(flyid++,".$id.",'assignments_add.php');\"");
//	$xt->assign("inlineedit_column",$allow_edit);
}
...
$xt->assign("Marks_fieldheadercolumn",true);
$xt->assign("Marks_fieldcolumn",true);
$xt->assign("Marks_fieldfootercolumn",true);
...
$xt->assign("asearchlink_attrs","href=\"assignments_search.php\" onclick=\"window.location.href='assignments_search.php';return false;\"");
$xt->assign("import_link",$allow_import);
$xt->assign("importlink_attrs","href=\"assignments_import.php\" onclick=\"window.location.href='assignments_import.php';return false;\"");
...
if($display_grid)
{
	if($mode==LIST_SIMPLE)
		...
		"<form method=\"POST\" action=\"assignments_list.php\" name=\"frmAdmin\" id=\"frmAdmin\">
		<input type=\"hidden\" id=\"a\" name=\"a\" value=\"delete\">",
		"</form>");
	elseif($mode==LIST_LOOKUP)
		$xt->assign_section("grid_block",
		"<form method=\"POST\" action=\"assignments_list.php\" name=\"frmAdmin".$id."\" id=\"frmAdmin".$id."\" target=\"flyframe".$id."\">
...
if($mode==LIST_SIMPLE)
{
	...
	$xt->assign("recordspp_attrs","onchange=\"javascript: document.location='assignments_list.php?pagesize='+this.options[this.selectedIndex].value;\"");
	$xt->assign("pages_block",$allow_search && $rowsfound);
}
else
	$xt->assign("recordspp_attrs","onchange=\"javascript: window.frames['flyframe".$id."'].location='assignments_list.php?".$lookupparams."&pagesize='+this.options[this.selectedIndex].value;\"");
...
$templatefile = "assignments_list.htm";
