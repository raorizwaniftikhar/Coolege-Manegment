...
$body=array();
$body["begin"]="<form action=\"users_export.php\" method=get id=frmexport name=frmexport>";
$body["end"]="</form>";
$xt->assignbyref("body",$body);
$xt->display("users_export.htm");
...
function ExportToXML()
{
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("UID"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"UID",""));
		echo "</".$field.">\r\n";
		$field=htmlspecialchars(XMLNameEncode("PWD"));
		echo "<".$field.">";
		echo htmlspecialchars(GetData($row,"PWD",""));
		echo "</".$field.">\r\n";
...
// write data rows
	...
	while((!$nPageSize || $iNumberOfRows<$nPageSize) && $row)
	{
		...
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"UID",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
		$outstr.='"'.htmlspecialchars(GetData($row,"PWD",$format)).'"';
		if($outstr!="")
			$outstr.=",";
			$format="";
...
// write header
	...
	if($_REQUEST["type"]=="excel")
	{
		...
		echo '<td style="width: 100" x:str>'.PrepareForExcel("UID").'</td>';
		echo '<td style="width: 100" x:str>'.PrepareForExcel("PWD").'</td>';
...
// write data rows
	...
	if($_REQUEST["type"]=="excel")
		...
		$format="";
			if($_REQUEST["type"]=="excel")
			echo PrepareForExcel(GetData($row,"UID",$format));
		else
			echo htmlspecialchars(GetData($row,"UID",$format));
	echo '</td>';
	if($_REQUEST["type"]=="excel")
		echo '<td x:str>';
	else
		...
		$format="";
			if($_REQUEST["type"]=="excel")
			echo PrepareForExcel(GetData($row,"PWD",$format));
		else
			echo htmlspecialchars(GetData($row,"PWD",$format));
	echo '</td>';
	echo '<td>';
...
function ExportToPDF()
{
	...
	function AcceptPageBreak()
	{
		...
		$x+=$colwidth["ID"];
		$this->Rect($x,$this->y0,$colwidth["UID"],$this->maxheight);
		$x+=$colwidth["UID"];
		$this->Rect($x,$this->y0,$colwidth["PWD"],$this->maxheight);
		$x+=$colwidth["PWD"];
...
//		$this->Cell($colwidth["ID"],$this->rowheight,"ID",1,0,'C',1);
		$this->Cell($colwidth["ID"],$this->rowheight,"ID",1,0,'C',1);
//		$this->Cell($colwidth["UID"],$this->rowheight,"User ID",1,0,'C',1);
		$this->Cell($colwidth["UID"],$this->rowheight,"User ID",1,0,'C',1);
//		$this->Cell($colwidth["PWD"],$this->rowheight,"Password",1,0,'C',1);
		$this->Cell($colwidth["PWD"],$this->rowheight,"Password",1,0,'C',1);
//		$this->Cell($colwidth["Type"],$this->rowheight,"Type",1,0,'C',1);
    ...
    $colwidth["UID"]=$defwidth;
    $colwidth["PWD"]=$defwidth;
	...
	while((!$nPageSize || $i<$nPageSize) && $row)
	{
		...
		$pdf->SetY($pdf->y0);
		$pdf->SetX($x);
		$pdf->MultiCell($colwidth["UID"],$rowheight,GetData($row,"UID",""));
		$x+=$colwidth["UID"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
		$pdf->SetY($pdf->y0);
		$pdf->SetX($x);
		$pdf->MultiCell($colwidth["PWD"],$rowheight,GetData($row,"PWD",""));
		$x+=$colwidth["PWD"];
		if($pdf->GetY()-$pdf->y0>$pdf->maxheight)
			$pdf->maxheight=$pdf->GetY()-$pdf->y0;
		$pdf->SetY($pdf->y0);
...
//	draw fames
		...
		$x+=$colwidth["ID"];
		$pdf->Rect($x,$pdf->y0,$colwidth["UID"],$pdf->maxheight);
		$x+=$colwidth["UID"];
		$pdf->Rect($x,$pdf->y0,$colwidth["PWD"],$pdf->maxheight);
		$x+=$colwidth["PWD"];
